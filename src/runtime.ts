...
export const QA_OPENID_CONFIG_URL = 'https://ai-awsfqa.avlr.sh/.well-known/openid-configuration';
export const FALLBACK_TOKEN_URL = 'https://identity.avalara.com/connect/token';
export const FALLBACK_DEVICE_AUTHORIZATION_URL = 'https://identity.avalara.com/connect/deviceauthorization';
...
export class ApiClient {
    ...
    protected async request(context: RequestOpts, initOverrides?: RequestInit, requiredScopes: string = '', isRetry: boolean = false): Promise<ResponseWithLogObject> {
        const logObject = new LogObject(this.logger.logRequestAndResponseInfo);
        const { url, init, timeoutId } = this.createFetchParams(context, logObject, initOverrides);
    ...
    private async updateOAuthAccessToken(scopes, accessToken) {
        ...
        if (!currentAccessToken || currentAccessToken === accessToken) {
            try {
                ...
                let timestamp = Math.floor(Date.now() / 1000) + tokenResponse['expires_in'];
                this.accessTokenMap.set(scopes, { accessToken: tokenResponse['access_token'], expiry: timestamp });
            } catch (err) {
    ...
    private createFetchParams(context: RequestOpts, logObject: LogObject, initOverrides?: RequestInit) {
        ...
        logObject.populateRequestInfo(url, { body: context.body, method: context.method });
    ...
    private fetchApi = async (url: RequestInfo, init?: RequestInit, logObject?: LogObject) => {
...
export class Configuration {
    ...
    public async setOIDCUrls() {
        const { environment, testTokenUrl, testDeviceAuthorizationUrl } = this.configuration;
        if (environment === AvaTaxEnvironment.Test && testTokenUrl) {
            (this.oidcUrlObject || {} as OIDCMetadata).tokenUrl = testTokenUrl;
            (this.oidcUrlObject || {} as OIDCMetadata).deviceAuthorizationUrl = testDeviceAuthorizationUrl;
            ...
            try {
                ...
                console.log(`Exception when calling OpenIdConnect URL: ${this.getOpenIdConnectUrl} to fetch the token and device auth endpoint. Error: ${ex.message}. Defaulting to Fallback Token URL.`);
                this.oidcUrlObject = {
...
export type ResponseWithLogObject = { response: Response, logObject: LogObject}
